...
class CLanguage(Language):
    ...
    def output_templates(self, f):
        ...
        if not parameters:
                ...
                if p.is_vararg():
                    if not new_or_init:
                        parser_code.append(normalize_snippet("""
                            ...
                            """ % (p.converter.parser_name, left_args, left_args, p.converter.parser_name, max_pos), indent=4))
                    else:
                        parser_code.append(normalize_snippet("""
                            %s = PyTuple_GetSlice(%d, -1);
                            """ % (p.converter.parser_name, max_pos), indent=4))
                    continue
            ...
            if vararg == NO_VARARG:
                args_declaration = "_PyArg_UnpackKeywords", "%s, %s, %s" % (min_pos, max_pos, min_kw_only)
            else:
                args_declaration = "_PyArg_UnpackKeywordsWithVararg", "%s, %s, %s, %s" % (min_pos, max_pos, min_kw_only, vararg)
            if not new_or_init:
